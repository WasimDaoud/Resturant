...
const Meals = () => {
  const mealsKind = useSelector((state) => state.meals);

  const [searchMeals, setSearchMeales] = useState(mealsKind);

  const searching = (e) => {
    const val = e.target.value;
    const search = mealsKind.filter((meal) => {
      return meal.title.toLowerCase().includes(val.toLowerCase());
    });

    if (val != "") {
      setSearchMeales(search);
    } else {
      setSearchMeales(mealsKind);
    }
  };
