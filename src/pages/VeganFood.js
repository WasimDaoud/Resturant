...
const VeganFood = () => {
  ...
  const vegans = useSelector((state) => state.sweetVegan);
  const [searchVegans, setSearchVegans] = useState(vegans);

  const searching = (e) => {
    const val = e.target.value;
    const search = vegans.filter((meal) => {
      return meal.title.toLowerCase().includes(val.toLowerCase());
    });

    if (val != "") {
        setSearchVegans(search);
    } else {
      setSearchVegans(vegans);
    }
  };
